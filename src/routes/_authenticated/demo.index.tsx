...
import { computeClipScore, useIncome, useProfile, useSusu } from "@/lib/demo-queries";
...
function DemoHome() {
  ...
  const todayTotal = income.filter((i) => i.entry_date === today).reduce((s, i) => s + Number(i.amount), 0);
  const weekTotal = income.reduce((s, i) => s + Number(i.amount), 0);
  const myPot = groups.filter((g) => g.joined).reduce((s, g) => s + Number(g.pot), 0);
  ...
  return (
    <div className="pt-4 space-y-5">
      <div>
        ...
        <h1 className="text-2xl font-display font-bold">{profile?.display_name || "Welcome"}</h1>
        <div className="text-xs text-muted-foreground">{profile?.business_name || "Your shop"}</div>
      </div>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-primary/25 to-gold/10 border border-primary/30 p-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="text-[11px] text-primary mt-1">GH₵ {weekTotal.toFixed(0)} total logged</div>
          </div>
      ...
      <div className="rounded-2xl bg-surface-elevated border border-border p-5 flex items-center justify-between">
        <div>
          ...
          <div className="text-[11px] text-muted-foreground mt-1 max-w-[140px]">Built from your logged records. Higher score = bigger loans.</div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Link to="/demo/loans" className="rounded-2xl bg-surface-elevated border border-border p-4 hover:border-primary/50 transition">
          ...
          <div className="text-lg font-bold">GH₵ {loanBalance.toFixed(0)}</div>
          <div className="text-[10px] text-gold">No active loan</div>
        </Link>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <Link key={a.to} to={a.to as "/demo"} className="rounded-xl bg-surface-elevated border border-border p-3 text-center">
      ...
      <div className="rounded-2xl border border-gold/30 bg-gold/10 p-4">
        ...
        <div className="text-sm mt-1">Log income every day for 7 days straight to grow your ClipScore.</div>
      </div>
    </div>
  );
}
