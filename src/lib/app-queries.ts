...
export function useApplyLoan() {
  ...
  return useMutation({
    mutationFn: async (v: { amount: number; duration_days: number; purpose: string }) => {
      ...
      // 1. Fetch current interest rate from system settings
      ...
      const rateMultiplier = 1 + (Number(settings.interest_rate) / 100);

      // 2. Insert loan application with dynamic interest
      const { data, error } = await supabase.from("loan_applications").insert({
        ...
        term_months: Math.ceil(v.duration_days / 30)
      }).select().single();
